function Hearing_Model_Final
    ...
    % Status: Fixed plotting overlap bug & Enhanced UI aesthetics.
    ...
    % Deep Navy Backgrounds (Easier on eyes than pure black)
    ...
    ax.GridAlpha = 0.4; % Make grid slightly visible
    ax.Box = 'on';
    grid(ax, 'on'); 
    ax.FontSize = 12;
    ax.XScale = 'log'; 
    ...
    % 1. SLIDER
    uilabel(pnl, 'Position', [30 110 150 20], ...
        'Text', 'Polynomial Degree:', ...
        'FontWeight', 'bold', 'FontColor', col.text);
    
    sld = uislider(pnl, ...
        'Position', [190 120 450 3], ...
        'Limits', [1 12], ...
        'Value', 2, ...
        'MajorTicks', 1:12, ...
        'MinorTicks', [], ...
    ...
    % Dynamic Label (Degree)
    lblDegree = uilabel(pnl, 'Position', [190 80 450 30], ...
    ...
    % 3. ERROR DISPLAY (RMSE)
    % Placed nicely inside the plot area (top right)
    lblRMSE = uilabel(fig, 'Position', [730 590 180 30], ...
        'Text', 'RMSE: ...', ...
        'HorizontalAlignment', 'center', ...
        'BackgroundColor', [0.1 0.1 0.1 0.7], ... % Semi-transparent bg
    ...
    function updateGraph(slider, axes, x, x_real, y, colors)
        ...
        % B) Linear Algebra (Least Squares)
        n = length(x);
        A = zeros(n, degree + 1);
        ...
        % D) Smooth Curve Generation
        x_smooth = linspace(min(x), max(x), 500); % High res for smoothness
        y_smooth = polyval(flipud(c), x_smooth);
        
        % E) PLOTTING FIX (Preventing Overlap)
        
        % IMPORTANT: Clear the axes canvas before drawing new frame
        hold(axes, 'off'); 
        
        % Step 1: Draw the FIT LINE first (so it stays behind dots)
        plot(axes, 10.^x_smooth, y_smooth, '-', ...
            'Color', colors.line, ...
            'LineWidth', 2.5); 
        
        hold(axes, 'on'); % Keep the line, add dots
        
        % Step 2: Draw DATA POINTS on top
        plot(axes, x_real, y, 'o', ...
            'MarkerSize', 7, ...
            'MarkerFaceColor', colors.dots, ...
            'MarkerEdgeColor', 'white', ... % White outline makes it pop
            'LineWidth', 0.8);
            
        hold(axes, 'off'); % Release hold
        
        % F) Graph Settings (Must re-apply after plot)
    ...
    function handleAudio(btn, figureHandle, colors)
        ...
        % Sync Button State: When audio stops (naturally or forced), reset button
